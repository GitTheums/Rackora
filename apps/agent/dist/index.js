...
import { AGENT_HEARTBEAT_INTERVAL_MS, agentInfoSchema, RACKORA_VERSION, } from "@rackora/shared";
import { createBackoff, nextBackoffDelay, resetBackoff } from "./backoff.js";
import { createAgentHttpClient } from "./client.js";
import { loadAgentConfig } from "./config.js";
import { loadCredentials, saveCredentials, } from "./credentials.js";
import { createDockerSocketClient } from "./docker/client.js";
import { createTelemetryCollector, } from "./telemetry/collect.js";
export function getAgentInfo(status = "idle") {
...
async function ensureEnrolled(config, client) {
    const existing = loadCredentials(config.DATA_DIR);
    if (existing) {
        return existing;
    }
    if (!config.ENROLLMENT_TOKEN) {
        throw new Error("No credentials in DATA_DIR and ENROLLMENT_TOKEN is not set");
    }
...
function resolveDockerClient(options, config) {
...
async function createMinimalTelemetry(agentName, sleep) {
